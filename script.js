document.addEventListener('DOMContentLoaded', () => {
    ...
    if (activityH1) {
        const words = activityH1.textContent.trim().split(/\s+/);
        activityH1.innerHTML = '';
        let charIndex = 0;
        words.forEach((word) => {
            const wordSpan = document.createElement('span');
            wordSpan.className = 'flame-word';
            for (let i = 0; i < word.length; i++) {
                const span = document.createElement('span');
                span.className = 'flame-char';
                span.textContent = word[i];
                const delay = ((charIndex * 0.12) % 1.4).toFixed(2);
                const duration = (1.4 + Math.sin(charIndex * 0.7) * 0.4).toFixed(2);
                span.style.animationDelay = delay + 's';
                span.style.animationDuration = duration + 's';
                wordSpan.appendChild(span);
                charIndex++;
            }
            activityH1.appendChild(wordSpan);
